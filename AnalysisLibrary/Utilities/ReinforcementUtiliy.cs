...
namespace TempAnalysis.Utilities
{
    ...
    public class ReinforcementUtility
    {
        ...
        public List<double> GetAvailableThicknessFromWallLength(double wallLength, double FloorHeight)
        {
        ...
        public SpecialShearWallReinforcement GetShearWallSectionForFirstFloor(ShearWallAppliedForces Forces,double MaxWallLength,double FloorHeight, ConCreteMaterial shearWallMaterial, SteelMaterial MainSteel, SteelMaterial MildSteel, SpecialShearWallReinforcement StartSection, double incrt)
        {
            ...
            double Length = StartSection.Length;
            ...
            foreach (double WallLength in Lengthes)
            {
                ...
                {
                    if (Math.Abs(WallLength - StartSection.Length) < 1e-9 && Math.Abs(Thickness - StartSection.Thickness) < 1e-9)
                        continue;
        ...
        public SpecialShearWallReinforcement GetShearWallSectionForNextFloor(ShearWallAppliedForces Forces, double MaxWallLength, double FloorHeight, ConCreteMaterial shearWallMaterial, SteelMaterial MainSteel, SteelMaterial MildSteel, SpecialShearWallReinforcement PreviousSection, double Increment)
        {
        ...
        public InteractionDiagram CreateInteractionDiagram(IRectSection section, ConCreteMaterial shearWallMaterial, SteelMaterial mainSteel, bool CompressionOnly= false)
        {
            ...
            for (int i = 0; i <= Size; i++)
            {
                ...
                //ND.A_To_B_points.Add(new Point2D(0.9 * M2, 0.9 * N2));
            }
            if (CompressionOnly)
            {
