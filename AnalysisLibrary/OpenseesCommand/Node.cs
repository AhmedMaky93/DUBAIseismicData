...
namespace TempAnalysis.OpenseesCommand
{
    ...
    public class Node2D : BaseCommand
    {
        ...
        public Node2D(IDsManager IDM, double Z, double mass =0)
        {
            ID = ++IDM.LastNodeId;
            this.Z = Z;
            this.Mass = mass;
        ...
        {
            return $"-mass {Ts(Mass)} 0.0 {Ts(Mass)} 0.0  0.0  0.0";
        }
        public void WriteMassCommand(StreamWriter writer)
        {
            if(Mass > 1e-10)
                writer.WriteLine($"node {ID} 0.0 {Z} 0.0  {GetMassString()};");
            else
                writer.WriteLine($"node {ID} 0.0 {Z} 0.0;");
        }
        public override void WriteCommand(StreamWriter writer)
        {
            WriteMassCommand(writer);
        }
        public void WriteFixCommand(StreamWriter writer)
        {
            writer.WriteLine($"fix {ID} 0 1 0 0 1 0;");
        }
    }
    [DataContract(IsReference = true)]
