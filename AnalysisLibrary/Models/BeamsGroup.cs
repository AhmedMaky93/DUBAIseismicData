...
namespace TempAnalysis.Models
{
    ...
    public class BeamsGroup
    {
        ...
        [DataMember]
        public List<FrameElasticElement> ElasticSideElements = new List<FrameElasticElement>();
        [DataMember]
        ...
        public BeamsGroup(BeamsGroup grp):this(new BeamSection(grp.CouplingBeamSection))
        {
        }
        public void CreateElements(IDsManager IDM, Concrete02Material concrete01Material, SteelUniAxialMaterial steelMaterial, List<List<BaseNode>> Locations, string name, double BeamLength)
        {
            ...
            ElasticSectionProperties elasticSectionProperties = CouplingBeamSection.GenerateElasticProperties(1,concrete01Material,steelMaterial);
            ElasticSectionProperties SideElasticSectionProperties = CouplingBeamSection.GenerateElasticProperties(1e5, concrete01Material,steelMaterial);
            CoreElements = new List<FrameNonLinearElement>();
            ElasticCoreElements = new List<FrameElasticElement>();
            ElasticSideElements = new List<FrameElasticElement>();
            foreach (List<BaseNode> beamNodes in Locations)
            {
                ...
                ElasticSideElements.Add(new FrameElasticElement(IDM, beamNodes[0], beamNodes[2], 2, SideElasticSectionProperties));
                ElasticSideElements.Add(new FrameElasticElement(IDM, beamNodes[2], beamNodes[4], 2, SideElasticSectionProperties));
                
                ElasticSideElements.Add(new FrameElasticElement(IDM, beamNodes[1], beamNodes[3], 2, SideElasticSectionProperties));
                ElasticSideElements.Add(new FrameElasticElement(IDM, beamNodes[3], beamNodes[5], 2, SideElasticSectionProperties));
            }
            if (Locations.Any())
            { 
        ...
        public void WriteCommands(StreamWriter file, bool Elastic)
        {
            ...
            ElasticSideElements.ForEach(x => x.WriteCommand(file));
