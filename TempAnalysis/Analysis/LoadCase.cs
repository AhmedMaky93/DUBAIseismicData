...
namespace TempAnalysis
{
    ...
    public class ModalAnalysisLoadCase  : LoadCase
    {
        ...
        public override void ReadResults(IModalAnalysisModel model, LoadCombinationFactors factors, OpenSeesTranslator.OpenSeesTranslator translator)
        {
            ...
            CalculateFloorForces(model);
            model.PostCalculateFloorForces();
        }

        internal double GetTa(IModalAnalysisModel model)
        {
            return 0.03 * Math.Pow(model.GetH(), 0.9);
        }
        private void CalculateFloorForces(IModalAnalysisModel model)
        {
            CalculateFloorLevelForces(model);
            SetVdandMdValues(model);
        }
        public void CalculateFloorLevelForces(IModalAnalysisModel model)
        {
            double ratio = 0.01 * 9.81;
            double Ta = GetTa(model);
            double CuTa = 1.4 * Ta;

            model.GetModelFloors().ForEach(f =>
            {
                f.MFs = new List<double>();
                model._ModeShapes.ForEach(mode =>
                {
                    double sa = model._ResponseParameters.GetSa(Math.Min(mode.Period, CuTa));
                    f.MFs.Add(mode.X_values[f.Index - 1] * sa * mode.Mx * f.GetFloorMass() * ratio);
                });
            });
        }
        private void SetVdandMdValues(IModalAnalysisModel model)
        {
            double FloorHeight = model.GetH() / model._NumOfFloors;
            int last = model._NumOfFloors;
            List<IStructuralFloor> Floors = model.GetModelFloors();

            Floors.ForEach(f => {
                f.MVs = new List<double>();
                f.MMs = new List<double>();

                int start = f.Index - 1;
                for (int j = 0; j < model._ModeShapes.Count; j++)
                {
                    double Vd = 0;
                    double Md = 0;
                    ...
                    {
                        Vd += Floors[i].MFs[j];
                        Md += Floors[i].MFs[j] * (i - start + 1) * FloorHeight;
                    }
                    f.MVs.Add(Vd);
                    f.MMs.Add(Md);
                }

                f.Vd = Math.Sqrt(f.MVs.Sum(x => Math.Pow(x, 2)));
                f.Md = Math.Sqrt(f.MMs.Sum(x => Math.Pow(x, 2)));
            });
        }
        private void ReadModesResults(IModalAnalysisModel model, OpenSeesTranslator.OpenSeesTranslator Translator)
        {
            int incr = (model._3DModel ? 3 : 1);
            ...
            foreach (var mode in model._ModeShapes)
            {
                ...
                for (int i = 0; i < Results.Count; i += incr)
                {
                    ...
                    mode.Y_values.Add(model._3DModel? Results[i+2] : 0.0);
                }
                //mode.Normalize();
            }
        }
        private void ReadModalAnalysisReport(IModalAnalysisModel model, OpenSeesTranslator.OpenSeesTranslator Translator)
        {
            ...
            using (var fs = new FileStream(filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
                using (var sr = new StreamReader(fs, Encoding.Default))
                {
                    ...
                    line = sr.ReadLine();
                    lineRecords = model._3DModel ? 3 : 1;
                    if (!ConvertLineToDoubleList(line, lineRecords, out doubleList))
                        return;
                    ...
                    line = sr.ReadLine();
                    line = sr.ReadLine();
                    lineRecords = model._3DModel?4:2;
                    for (int i = 0; i < modeShapes; i++)
                    {
                        line = sr.ReadLine();
                        if (!ConvertLineToDoubleList(line, lineRecords, out doubleList))
        ...
        public bool IsEnoughModeShapes(IModalAnalysisModel model)
        {
            ...
            if (LastMode.SumMx < Max)
                return false;
            return model._3DModel? Math.Abs(LastMode.SumMx - LastMode.SumMy) < Tolerance : true;
        }
        public override void WriteAnalysisCommand(IModalAnalysisModel model, OpenSeesTranslator.OpenSeesTranslator Translator, LoadCombinationFactors factors)
        {
            string filePath = Path.Combine(Translator.FolderPath, Translator.AnalysisFile);
        ...
        private void WriteModalAnalysisCommands(StreamWriter file, IModalAnalysisModel model, OpenSeesTranslator.OpenSeesTranslator Translator)
        {
            ...
            file.WriteLine(string.Format("  recorder Node -file [format \"mode%i.out\" $k] -region {0} -dof {1} \"eigen $k\"", model._MasterNodesRegion.ID, model._3DModel ? "1 2 3":"1"));
            file.WriteLine("}");
            ...
            file.WriteLine(model._3DModel? Properties.Resources.ModalAnalysisScript3D : Properties.Resources.ModalAnalysisScript);
        }
    }
    ...
    public class PushOverLoadCase : LoadCase
    {
        ...
        private void WritepushOverAnalysisCommand(IModalAnalysisModel model,StreamWriter file)
        {
            ...
            if (Power >= 0)
            {
                ...
                {
                    string Loads = model._3DModel ? "0.0 0.0 0.0 0.0" : "";
                    double H = ((i - bottom) + 1) * FloorHeight;
                    file.WriteLine($"load {i} {1.0 * Math.Pow(H / Height, Power)} 0.0 {Loads};");
                }
            }
            else
            {
                List<double> MFs = model.GetModalForces();
                for (long i = bottom; i <= Top; i++)
                {
                    string Loads = model._3DModel ? "0.0 0.0 0.0 0.0" : "";
                    int index = (int)(i -bottom);
                    file.WriteLine($"load {i} {0.1 * MFs[index]} 0.0 {Loads};");
                }
            }
            ...
            file.WriteLine($"test NormDispIncr 1e-6 100;");
